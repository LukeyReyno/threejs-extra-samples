...
async function init() {
    ...
    const files = await (await fetch(`${RELATIVE_DIR}/files.json`)).json();
    const tags = await (await fetch(`${RELATIVE_DIR}/tags.json`)).json();
    ...
    for (const key in files) {
        ...
        for (let i = 0; i < category.length; i++) {
            ...
            validRedirects.set(file, `${RELATIVE_DIR}/${file}.html`);
        }
    }
...
function createLink(file, tags) {
    ...
    const template = `
        <div class="card">
            <a href="${RELATIVE_DIR}/${file}.html" target="viewer">
                <div class="cover">
                    <img src="${RELATIVE_DIR}/screenshots/${file}.jpg" loading="lazy" width="400" />
                </div>
